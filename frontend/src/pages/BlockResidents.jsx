...
const inputStyle = { width: '100%', minHeight: 40, padding: '8px 10px', border: '1px solid #CBD5E1', borderRadius: 9, color: '#1A3A5C' };
...
function ResidentSummary({ resident, action, actionLabel, actionDisabled = false, secondaryAction }) {
  ...
  return (
    <li data-testid={`block-resident-${resident.id}`} style={{ listStyle: 'none', border: '1px solid #E2E8F0', borderRadius: 11, padding: 12, background: '#fff' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
        <div style={{ minWidth: 0 }}>
          <strong style={{ display: 'block', color: '#1A3A5C', fontSize: 13 }}>{resident.displayName || resident.name}</strong>
          ...
          {workload && <div style={{ marginTop: 8, color: '#475569', fontSize: 11, lineHeight: 1.55 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          {secondaryAction}
          {action && <button type="button" disabled={actionDisabled} onClick={action} style={{ ...buttonBase, minHeight: 34, padding: '6px 9px', border: '1px solid #CBD5E1', background: actionDisabled ? '#F8FAFC' : '#fff', color: actionDisabled ? '#94A3B8' : '#2C5F8A' }}>{actionLabel}</button>}
        </div>
      </div>
    </li>
  );
...
function DateListEditor({ label, dates, onChange }) {
  ...
  return <div><label style={{ display: 'block', color: '#334155', fontSize: 12, fontWeight: 700, marginBottom: 6 }}>{label}</label><div style={{ display: 'flex', gap: 7 }}><input type="date" aria-label={`Add ${label.toLowerCase()}`} value={nextDate} onChange={event => setNextDate(event.target.value)} style={inputStyle} /><button type="button" onClick={() => { if (nextDate && !keys.includes(nextDate)) onChange([...keys, nextDate]); setNextDate(''); }} style={{ ...buttonBase, border: '1px solid #CBD5E1', background: '#fff', color: '#2C5F8A' }}>Add</button></div>{keys.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>{keys.map(date => <button key={date} type="button" aria-label={`Remove ${label.toLowerCase()} ${date}`} onClick={() => onChange(keys.filter(item => item !== date))} style={{ border: 0, borderRadius: 999, padding: '5px 8px', background: '#F1F5F9', color: '#475569', fontSize: 11 }}>{date} ×</button>)}</div>}</div>;
}
...
function AvailabilityModal({ resident, blockId, onClose, onSaved }) {
  ...
  return <Modal title={`Availability · ${resident.displayName || resident.name}`} description="This block only" onClose={onClose} maxWidth="max-w-xl" footer={<div style={{ display: 'flex', gap: 8 }}><button type="button" onClick={onClose} style={{ ...buttonBase, flex: 1, background: '#fff', border: '1px solid #CBD5E1' }}>Cancel</button><button type="button" onClick={save} disabled={saving} data-testid="save-block-availability" style={{ ...buttonBase, flex: 1, background: '#1A3A5C', color: '#fff', border: 0 }}>{saving ? 'Saving…' : 'Confirm availability'}</button></div>}>
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div><label style={{ display: 'block', color: '#334155', fontSize: 12, fontWeight: 700, marginBottom: 6 }}>Academic time</label><AcademicTimeEditor value={academicTimes} onChange={setAcademicTimes} /></div>
      <div><label htmlFor="local-call-cap" style={{ display: 'block', color: '#334155', fontSize: 12, fontWeight: 700, marginBottom: 6 }}>Local call maximum (optional)</label><input id="local-call-cap" type="number" min="1" value={callCapOverride} onChange={event => setCallCapOverride(event.target.value)} style={inputStyle} /></div>
    </div>
  </Modal>;
}
...
export default function BlockResidents() {
  ...
  return <Layout>
    <header style={{ marginBottom: 20 }}><button type="button" onClick={() => navigate(`/blocks/${block.number}`)} style={{ border: 0, background: 'transparent', color: '#2C5F8A', fontWeight: 700, padding: '5px 0' }}>← Block {block.number}</button><h1 style={{ margin: '7px 0 0', color: '#1A3A5C', fontSize: 28 }}>Manage block residents</h1><p style={{ margin: '6px 0 0', color: '#64748B', fontSize: 14 }}>Choose who is rotating, then confirm each resident’s block availability.</p></header>

    {readiness && <section data-testid="block-resident-readiness" style={{ border: `1px solid ${needsAttention ? '#FCD34D' : '#86EFAC'}`, background: needsAttention ? '#FFFBEB' : '#F0FDF4', borderRadius: 12, padding: 14, marginBottom: 18 }}>
      <strong style={{ color: needsAttention ? '#92400E' : '#166534' }}>{needsAttention ? 'Needs attention' : 'Ready to generate'}</strong>
      <div style={{ marginTop: 5, color: '#475569', fontSize: 12 }}>{composition?.counts?.total ?? 0} active residents in this block · {readiness.residents.withAvailability}/{readiness.residents.inBlockTotal} have availability configured · {readiness?.vacation?.list?.length ?? 0} residents have vacation · {unresolved.length} unresolved resident records</div>
      {missing.length > 0 && <div style={{ marginTop: 6, fontSize: 12, color: '#92400E' }}>Missing: {missing.map(item => item.residentName).join(', ')}</div>}
      {unresolved.length > 0 && <div style={{ marginTop: 6, fontSize: 12, color: '#92400E' }}>Resolve: {unresolved.map(item => item.residentName).join(', ')}</div>}
    </section>}

    <div className="block-transfer-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 16 }}>
      <section style={{ border: '1px solid #E2E8F0', borderRadius: 14, background: '#F8FAFC', padding: 14 }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 12 }}><div><h2 style={{ margin: 0, color: '#1A3A5C', fontSize: 16 }}>Available residents</h2><span style={{ color: '#64748B', fontSize: 11 }}>{available.length} directory records</span></div>{canCreate && <button type="button" onClick={() => setCreating(true)} style={{ ...buttonBase, background: '#fff', border: '1px solid #CBD5E1', color: '#2C5F8A' }}>+ New rotating resident</button>}</div><ul style={{ margin: 0, padding: 0, display: 'grid', gap: 8 }}>{available.length ? available.map(resident => <ResidentSummary key={resident.id} resident={resident} action={canManage ? () => add(resident) : null} actionLabel="Add →" />) : <li style={{ listStyle: 'none', color: '#64748B', fontSize: 13, padding: 12 }}>Everyone eligible is already in this block.</li>}</ul></section>
      <section style={{ border: '1px solid #BFDBFE', borderRadius: 14, background: '#EFF6FF', padding: 14 }}><div style={{ marginBottom: 12 }}><h2 style={{ margin: 0, color: '#1A3A5C', fontSize: 16 }}>In this block</h2><span style={{ color: '#64748B', fontSize: 11 }}>{composition?.counts?.inService ?? 0} in-service · {composition?.counts?.offService ?? 0} off-service · {composition?.counts?.medicalStudents ?? 0} medical students</span></div><ul style={{ margin: 0, padding: 0, display: 'grid', gap: 8 }}>{inBlock.map(resident => <ResidentSummary key={resident.id} resident={resident} action={canManage && (!resident.isServiceResident || resident.isMedStudent) ? () => remove(resident) : null} actionLabel="Remove" secondaryAction={canManage ? <button type="button" onClick={() => setEditingAvailability(resident)} style={{ ...buttonBase, minHeight: 34, padding: '6px 9px', border: `1px solid ${resident.availabilityConfirmed ? '#86EFAC' : '#FCD34D'}`, background: '#fff', color: resident.availabilityConfirmed ? '#166534' : '#92400E' }}>{resident.availabilityConfirmed ? 'Availability' : 'Configure'}</button> : null} />)}</ul></section>
    </div>
